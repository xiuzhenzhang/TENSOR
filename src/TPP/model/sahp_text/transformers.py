...
class TransformerEncoder(nn.Module):
    def __init__(self, num_marks, device, d_text_emb, d_input, d_rnn, d_hidden, n_layers, n_head, d_qk, d_v, dropout):
        """
    ...
    def forward(self, time_history, mark_history, text_emb_history, non_pad_mask, custom_mark_history=False):
        """
        ...
        """
        return self.encoder(time_history, mark_history, text_emb_history, non_pad_mask, custom_mark_history)
...
class Encoder(nn.Module):
    def __init__(self, num_marks, d_text_emb, d_input, d_hidden, n_layers, n_head, d_qk, d_v, dropout, device):
        """
        This function builds a Transformer encoder.
    ...
    def forward(self, time_history, mark_history, text_emb_history, non_pad_mask, custom_mark_history):
        """
        Encode the input continuous-time event stream using Transformer.
        ...
        if self.d_text_emb < self.d_input:
            text_emb = torch.nn.functional.pad(text_emb_history, (0, self.d_input - self.d_text_emb, 0, 0, 0, 0), mode='constant', value=0)  # [batch_size, seq_len, d_input]
        else:
            text_emb = text_emb_history[..., :self.d_input]  # [batch_size, seq_len, d_input]

        mingled_emb = time_emb + mark_emb + text_emb  # [batch_size, seq_len, d_input]
